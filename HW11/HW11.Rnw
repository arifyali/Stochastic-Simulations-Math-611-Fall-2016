...
boot.BCa <-
  function(x, th0, th, Stat, conf = .90) {
    ...
    th.jack <- numeric(n)
    for (i in 1:n) {
      th.jack[i] <- Stat(x[-i, ]) #unlike the class example, we are calculating the median of a function 
    }
...
bca_int = boot.BCa(x = rexp(1e4, rate = 2), th0 = median(x), th = boot.median ,Stat = median)$BCa
